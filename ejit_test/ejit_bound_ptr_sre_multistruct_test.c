...
static int drive_profile(uint32_t target, uint32_t cell, uint64_t *sink) {
  ...
  *sink ^= got;
  ...
  // wrapper while it is pending: a tight producer loop can starve a
  // low-priority fixed worker on some SRE schedulers and is not needed to make
  // progress.
  for (uint32_t round = 0; round < EJIT_BOUND_MULTI_T2_WAIT_ROUNDS; ++round) {
    ...
    if (stats.compileFailed || stats.publishFailed) {
      print_profile_state("T2-failed", target, cell,
                          EJIT_BOUND_MULTI_T1_SAMPLES, baseline, &stats);
      return 0;
    }
    if (stats.asyncCompiles >= baseline + 2u && stats.pendingEntries == 0 &&
      ...
      *sink ^= verify2;
      print_profile_state("T2-ready", target, cell,
                          EJIT_BOUND_MULTI_T1_SAMPLES + 2u, baseline, &stats);
      return 1;
    }
    if (round == 0u || ((round + 1u) % 1000u) == 0u)
      print_profile_state("T2-wait", target, cell, EJIT_BOUND_MULTI_T1_SAMPLES,
                          baseline, &stats);
  }

  ejit_taskpool_stats_t stats = {0};
  (void)ejit_taskpool_get_stats(&stats);
  print_profile_state("T2-timeout", target, cell, EJIT_BOUND_MULTI_T1_SAMPLES,
                      baseline, &stats);
  return 0;
}
...
static int run_producer(void) {
  ...
  uint64_t sink = 0;
  const uint32_t targets[6] = {BOUND_MULTI_ROOT0, BOUND_MULTI_ROOT0,
                               BOUND_MULTI_ROOT1, BOUND_MULTI_ROOT2,
                               BOUND_MULTI_ROOT3, BOUND_MULTI_ROOT4};
  const uint32_t cells[6] = {EJIT_BOUND_MULTI_CELL_A, EJIT_BOUND_MULTI_CELL_B,
  ...
  __atomic_store_n(&g_bound_multi_stage, BOUND_MULTI_COMPILED,
                   __ATOMIC_RELEASE);
  SRE_printf(
      "[BOUND-MULTI][core=%u] PASS five roots T2 Ready; root0 cells=%u/%u "
