...
//
// Do not call ejit_shutdown(): the owner worker must survive the shell return.
//
//===----------------------------------------------------------------------===//
...
#endif
#if !__has_attribute(ejit_const_after_init) || !__has_attribute(ejit_entry)
...
EJIT_SHARED_SECTION_ATTR volatile uint32_t g_const_done;
...
int test_ejit_period(uint8_t a, uint8_t b, uint8_t c, uint8_t d) {
  ...
  if (core == CONST_WORKER_CORE) {
    if (__atomic_load_n(&g_const_done, __ATOMIC_ACQUIRE) != 0u) {
  ...
  ejit_taskpool_print_compiled();
  __atomic_store_n(&g_const_done, 1u, __ATOMIC_RELEASE);
  SRE_printf("[CONST-AI] PASS scale=%u bias=%lld mode=%d sink=0x%llx\n",
             ...
             (unsigned long long)__atomic_load_n(&g_const_sink,
                                                  __ATOMIC_RELAXED));
  SRE_printf("[CONST-AI] return to core %u and run test_ejit_period again "
             "for function/module dumps\n",
             CONST_WORKER_CORE);
  return 0;
}
