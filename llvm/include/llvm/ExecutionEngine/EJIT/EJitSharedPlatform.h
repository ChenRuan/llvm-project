...
/// v6: each cache bucket carries a monotonic publishSeq word used by the
/// optional EJIT_SRE_TASKPOOL_NO_RECLAIM seqlock reader (load-only hot-hit path
/// with no per-hit read-token RMW). The field exists in every build for a
