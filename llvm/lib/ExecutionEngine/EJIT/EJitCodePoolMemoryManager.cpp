  ...
  void finalize(OnFinalizedFunction OnFinalized) override {
    ...
    runFinalizeActions(
        ...
        [this, OnFinalized = std::move(OnFinalized)](
          ...
          SmallPtrSet<EJitCodePoolManager *, 2> PendingPools;
          for (const ExecSegRange &R : ExecRanges) {
            ...
            if (R.Pool->usesBatchedPageSeal())
              PendingPools.insert(R.Pool);
          }
          for (EJitCodePoolManager *PendingPool : PendingPools)
            PendingPool->notePendingAllocation();
          {
#ifndef EJIT_FREESTANDING
...
void EJitCodePoolMemoryManager::allocate(const JITLinkDylib *JD, LinkGraph &G,
  ...
  [[maybe_unused]] const char *Placement =
      FarPool_ == &Pool ? "far" : "near";
  EJIT_DIAG_DEBUG(
