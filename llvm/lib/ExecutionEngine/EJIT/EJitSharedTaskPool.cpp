...
EJitSharedTaskPool::peerPrepareSlot(EJitSharedCacheBucket &B, uint32_t bucket,
  ...
  // Re-acquire bucket ownership and RE-VALIDATE the slot: a publish may have
  ...
  //
  // In the default token build, a read token is enough to keep publish from
  // changing the slot while we memoize executableCoreMask. In NO_RECLAIM, reads
  // deliberately do not increment readers, so this rare cold path takes the
  // write lock for the revalidation + memo-bit update. This keeps the hot hit
  // load-only while avoiding a race where a stale prepare could mark a newly
  // published slot as executable on this core.
#ifdef EJIT_SRE_TASKPOOL_NO_RECLAIM
  bucketWrite(B);
#else
  if (!bucketTryRead(B)) {
...
void EJitSharedTaskPool::releaseRead(uint32_t bucketIndex) {
  // NO_RECLAIM cache hits return this sentinel because no read token was taken.
  // The AOT wrapper may still call releaseRead unconditionally, so keep this a
  // silent fast no-op instead of emitting diagnostics on every hot hit.
