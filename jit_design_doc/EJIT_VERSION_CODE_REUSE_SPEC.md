...
- owner维护与逻辑请求/生命周期版本绑定的借用状态；前缀callback返回只结束
  当前阶段，不代表逻辑借用结束。最终读取完成后清除descriptor并恰好确认一次，
...
| borrowed请求 | 借用覆盖入组等待及所有后续读取，最终读取结束/取消确认后才能更新或释放 | 用户已确认延长，仅共享模式 |
...
- borrowed非代表长时间等待期间cancel/deactivate/更换对象：取消确认前禁止
  更新，确认后旧对象不会被读取，所有借用/dedup/admission终结不重复、不遗漏。
