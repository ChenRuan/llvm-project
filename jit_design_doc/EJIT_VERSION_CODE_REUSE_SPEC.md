...
本PR创建基线为spec5的 `1265b885456811ca33b525eac903a4e8be56f0a9`。
...
PR223的free_dim正独立适配到spec5，目前不是本PR创建基线的一部分。它可提供
“只在分析中求地址、只替换load结果”的基础，但其固定见证0/调用方字段同值承诺，
不能替代本方案的真实cell/TRP求值、逻辑生命周期及最终IR精确比较。不要把已有
arr_ind(cell/TRP)改为free_dim以冒充自动合并。待223独立合入spec5后，再更新本分支
基线并验证兼容，不在本PR重复带入另一份223实现。
...
借用结构体的生命周期、同 VA、更新时 deactivate/modify/reactivate 契约不变。
...
只为代表生成独立T1和计数器，并累计其真实T1 dispatch 64次，冻结成组profile。
...
- 代表冷却、退出或配置失效时，使用有界、可配置的无进展超时；撤销它的新T1
  ...
  晚到回调按组代际丢弃，NO_RECLAIM下不冒险复用仍可能被旧调用写入的counter内存。
  无合适代表/超过重试预算时释放名额，组保持明确失败或回退状态，不能无限占坑。
- 代表达到阈值后不再继续T1采样；等待发布按既有AOT回退政策。组profile ready、
...
hash 命中且已 published：再次检查当前逻辑版本/代际，走该版本原有 cache
Ready/PGO completion/icache 填表步骤，但不再分配或封同一物理页。
hash 命中但尚 RW/NX：作为该 CodeRecord 的独立 waiter，不能提前把地址交给 wrapper。
实际封页成功后逐个校验 waiter 的版本，合法者发布，失效者单独丢弃。
...
精确比较材料设置字节预算；预算不足只停止去重/丢弃候选索引，不破坏仍可执行代码。
不长期保留每个版本完整 LLVMContext，不让新缓存无限增长。
...
| may_const ranking/density | 实际采样收益和公共profile分别来自谁，真实代码代价多少？ | 保留实际测得的hits/sites/cycles并标注scope/profile_id/source；未采样成员不复制代表数据；公共profile与个体动态site映射未经证明不能混算removed_hits；物理range去重 |
...
| T1/采样/发布 | 每组只一个代表cell运行T1并真实采64次，其他成员AOT等待并跳过T1；组级admission，发布触发不另改 | 用户已确认只采代表；状态机必须配套 |
...
- 公共profile的guard/fallback在非代表cell上必须语义正确；代表样本不能当作所有
  cell的实际采样统计。verifier/audit/default模式各自按策略隔离，不交叉命中。
...
个人分支，不为“规格已上传”提前合并一个未完成的功能PR。最终整理为清晰的逻辑
提交；正式合入前按仓库约定squash，不重写spec5已有的209/219等历史。
