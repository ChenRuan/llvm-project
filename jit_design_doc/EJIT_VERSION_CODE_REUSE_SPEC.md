...
用户已确认同一完整逻辑身份可能跨核或迁移，不能采用固定核独占槽的假设。
因此本核准备成功仍不授权填全局共享槽；另一个核可能绕过首次miss。V1保持
共享槽和原wrapper，必须在填槽前获得该代码对全部潜在读取核的执行准备证明。
这是首次发布冷路径的必要协调，不是已经存在或已经验证过的平台能力。

平台若能提供覆盖整个执行域的权限及指令/数据缓存同步保证，可用其成功完成
结果作为证明；否则需在目标业务核实际执行准备并有界确认，不能在owner重复
调用本核enable_ex冒充其他核已完成，也不能只更换一条TLBI就宣称契约成立。
证明与code_id、真实exec/data范围、权限代际和执行域关联，不能只记函数名、
cell或裸fn指针。共用代码的多个成员可以复用同一有效证明，不重复封物理页。

首次执行域必须覆盖所有可能直接读槽的核，不能只统计已经调用过的核。执行域
内迁移无需每次调用新增检查；加入新核或失去旧权限证明时，先按已有生命周期
同步协议阻止其访问该槽，重新准备后才允许进入，不能运行中静默扩大核集合。
无法提供该约束或准备能力时，在启用共享快填前明确拒绝不支持配置。实际准备
超时/部分失败时槽保持未填，冷路径只能返回已在本核合法准备且重验过的指针，
否则AOT；按6.5有界重试并诊断，不能假报全部成员快速可执行、无限阻塞worker
或让未准备核先执行。0维和所有维度数均按同一跨核契约处理。

跨核发布模块动工前必须验证具体平台回调/执行域声明/确认通道及失败恢复方案；
新增共享确认字段时显式升级相应ABI。此协调不能偷偷加入稳定wrapper，也不
顺带改成核私有表或要求产品固定cell核亲和性。owner发布逻辑Ready、本核resolve
成功、全局槽可填是三个不同判断，不能合成一个“publish成功”布尔值。
...
| 跨核快填 | 同一身份允许跨核/迁移；填全局槽前证明全部潜在读取核已准备 | 用户确认跨核；首次发布协调待实现，不增加稳定wrapper判断 |
...
- 未执行个人T1的非代表核，owner先完成复用Ready发布，其首次调用仍完成本核
  准备和重验；准备失败不填槽。同一逻辑身份再由另一未准备核调用（含0维），
  全局槽必须在全读取核证明成立前保持未填，不能因首核成功而绕过准备。覆盖
  首次执行核迁移、全核准备中一核失败/超时、权限代际和执行域变化；失败不
  假报快速可执行，恢复有界，原稳定wrapper机器码不变。
