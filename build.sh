  ...
  if [ "$type" = "debug" ]; then
    if [ "$variant" = "static" ]; then
      ...
      cmake -S "${LLVM_SRC}" -B "${build_dir}" \
        ...
        -DEJIT_WORKER_SERVICE=${EJIT_WORKER_SERVICE} \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --worker-service) EJIT_WORKER_SERVICE=ON; EJIT_SRE_TASKPOOL=ON ;;
    --no-worker-service) EJIT_WORKER_SERVICE=OFF ;;
    --worker-service-stack-size=*) EJIT_SRE_TASKPOOL_WORKER_STACK_SIZE="${1#--worker-service-stack-size=}" ;;
    -h|--help)
      sed -n '2,29p' "$0"
      exit 0
      ;;
